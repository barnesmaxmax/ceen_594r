...
library("knitr")
```
# USTM resiliency visualizations
...
ustm_whole_rowsum <- read.dbf("data/01_ROWSUMS_TEMP_WHOLE.DBF")  #original unbroken network
ustm_broken_rowsum <- read.dbf("data/01_ROWSUMS_TEMP_50.DBF")    #altered broken network
ustm_map <- st_read("data/TAZ_WF_USTM/TAZ.shp")               #load shapefile
```
...
ustm_rowsum_map <- left_join(ustm_map, ustm_broken_rowsum, by = c("TAZID" = "TAZ"))
plot(ustm_rowsum_map[, "LNHBW"])
plot(ustm_whole_rowsum[, "LNHBW"])
```
...
ustm_change_broken1 <- list(
  "Original" = ustm_whole_rowsum,
  "Broken Link" = ustm_broken_rowsum
  ) %>%
...
ustm_broken_map <- left_join(ustm_map, ustm_change_broken1, by = c("TAZID" = "TAZ"))

# Create choropleth with 5 trip purposes
plot(ustm_broken_map[, c("LNHBO", "LNHBW", "LNREC", "LNHBC", "LNNHB")])
```
...
# Stitching data together
ustm_broken_map <- left_join(ustm_map, ustm_change_broken2, by = c("TAZID" = "TAZ"))
...
ustm_change_broken3 <- list(
  ...
  spread(Scenario, logsum, fill = NA) %>%
  mutate(
    difference = sqrt(((Original - `Broken Link`)^2)/nrow(ustm_whole_rowsum))
  ) %>%
  select(TAZ, purpose, difference) %>%
  spread(purpose, difference, NA)

# Stitching data together
ustm_broken_map <- left_join(ustm_map, ustm_change_broken3, by = c("TAZID" = "TAZ"))
...
ustm_change_broken4 <- list(
  ...
  spread(Scenario, logsum, fill = NA) %>%
  mutate(
    difference = (abs(`Broken Link` - Original)/nrow(ustm_whole_rowsum))
  ) %>%
  select(TAZ, purpose, difference) %>%
  spread(purpose, difference, NA)

# Stitching data together
ustm_broken_map <- left_join(ustm_map, ustm_change_broken4, by = c("TAZID" = "TAZ"))
...
ustm_change_broken5 <- list(
  ...
  spread(Scenario, logsum, fill = NA) %>%
  mutate(
    difference = (abs((Original - `Broken Link`)/`Broken Link`))*(1/nrow(ustm_whole_rowsum))
  ) %>%
  select(TAZ, purpose, difference) %>%
  spread(purpose, difference, NA)

# Stitching data together
ustm_broken_map <- left_join(ustm_map, ustm_change_broken5, by = c("TAZID" = "TAZ"))
...
kable(RMSE(ustm_change_broken1, parameter = ustm_whole_rowsum, type = "RMSE"))
## Total RMSE between full model and absolute error comparison
kable(RMSE(ustm_change_broken2, parameter = ustm_whole_rowsum, type = "RMSE"))
## Total RMSE between full model and RMSE comparison
kable(RMSE(ustm_change_broken3, parameter = ustm_whole_rowsum, type = "RMSE"))
## Total RMSE between full model and mean absolute error comparison
kable(RMSE(ustm_change_broken4, parameter = ustm_whole_rowsum, type = "RMSE"))
## Total RMSE between full model and mean absolute percentage comparison
kable(RMSE(ustm_change_broken5, parameter = ustm_whole_rowsum, type = "RMSE"))
```
...
ustm_map <- st_read("data/TAZ_WF_USTM/TAZ.shp")    
ustm_map <- st_transform(ustm_map, CRS("+proj=longlat +datum=WGS84"))

labels <- sprintf(
  "<strong>%s</strong><br/>%g Destination Choice Logsum",
  ustm_broken_rowsum$TAZ, ustm_broken_rowsum$LNHBW
) %>% lapply(htmltools::HTML)

m <- leaflet() %>% setView(lng = -111.69138, lat = 40.78847, zoom = 7) %>% addTiles() %>%
  addPolygons(data=ustm_map,weight=1,col = 'black',smoothFactor = 0.5,
    opacity = 1.0, fillOpacity = 1,fillColor = ~colorQuantile("Blues", ustm_broken_rowsum$LNHBW)(ustm_broken_rowsum$LNHBW),
...
labels <- sprintf(
  "<strong>%s</strong><br/>%g Destination Choice Logsum",
  ustm_change_broken1$TAZ, ustm_change_broken1$LNHBW
) %>% lapply(htmltools::HTML)

m <- leaflet() %>% setView(lng = -111.69138, lat = 40.78847, zoom = 7) %>% addTiles() %>%
  addPolygons(data=ustm_map,weight=1,col = 'black',smoothFactor = 0.5,
    opacity = 1.0, fillOpacity = 1,fillColor = ~colorQuantile("Blues", ustm_change_broken1$LNHBW)(ustm_change_broken1$LNHBW))
m
```
